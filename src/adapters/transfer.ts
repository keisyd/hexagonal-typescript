 ...
 * @param {DynamoRepositoryInstance<Todo>} repository - Dynamo database methods.
 */
const transfer = (repository: DynamoRepositoryInstance<Todo>) => async (
  id: string
) => {
  ///@port bloquear saldo da origem

  /// @adpater buscareVerificar 1 (debit)

  /// @adpater buscarVarcart 2 (credit)
...
const updateTodo = (
  ...
  try {
    ...
    const ExpressionAttributeValues = validateUpdateTodo(
      params,
      currObject,
      user
    );
